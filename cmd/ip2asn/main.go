...
func main() {
    ...
    format := "table"
    if jsonFlag { format = "json" } else if csvFlag { format = "csv" }
    ...
    case "csv":
        if outPath == "" {
            ...
            if err != nil { fatalf("failed to create output file: %v", err) }
            defer f.Close()
            w := csv.NewWriter(f)
            output.WriteCSV(w, results)
            w.Flush()
    ...
    case "json":
        if outPath == "" {
            ...
            enc.SetIndent("", "  ")
            if err := enc.Encode(results); err != nil { fatalf("failed to write JSON: %v", err) }
        } else {
            f, err := os.Create(outPath)
            if err != nil { fatalf("failed to create output file: %v", err) }
            defer f.Close()
